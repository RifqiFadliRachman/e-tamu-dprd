        ...
        <header class="bg-white flex justify-between items-center py-4 px-6 border-b">
            <div class="flex items-center">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-500">Melihat total list tamu, kunjungan kerja, kunjungan tamu, kunjungan lainnya</p>
                </div>
            </div>
        ...
        <main class="p-6 overflow-y-auto">
            ...
            <div class="grid grid-cols-4 gap-4 mb-6">
                ...
                <div class="bg-white shadow p-4 rounded-lg">
                    ...
                    <p class="text-2xl font-bold">{{ $allGuests->where('jenis_kunjungan', 'kunjungan_kerja')->count() }}</p>
                </div>
                <div class="bg-white shadow p-4 rounded-lg">
                    <p class="text-gray-500">Kunjungan Tamu</p>
                    <p class="text-2xl font-bold">{{ $allGuests->where('jenis_kunjungan', 'kunjungan_tamu')->count() }}</p>
                </div>
                <div class="bg-white shadow p-4 rounded-lg">
                    <p class="text-gray-500">Kunjungan Lainnya</p>
                    <p class="text-2xl font-bold">{{ $allGuests->whereNotIn('jenis_kunjungan', ['kunjungan_kerja', 'kunjungan_tamu'])->count() }}</p>
                </div>
            </div>
                ...
                <form action="{{ route('admin.dashboard') }}" method="GET">
                    <div class="relative w-full flex items-center">
                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                            <svg class="w-5 h-5 text-[#E8BF6F]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                        </div>
                        <input
                            type="text"
                            name="search"
